...
export default function ViewSQLTabContent(props: AnswerResultProps) {
  ...
  const onCopy = () => {
    if (!nativeSQLResult.nativeSQLMode) {
      message.success(
        <>
          You copied Rubix SQL. This dialect is for the Rubix Engine and may not
  ...
  return (
    <div className="text-md gray-10 p-6 pb-4">
      <Alert
        ...
        message={
          <>
            You’re viewing Rubix SQL by default. If you want to run this query on
          ...
          <div>
            {nativeSQLResult.nativeSQLMode ? (
              ...
              <span className="d-flex align-center gx-2">
                <Logo size={18} />
                <Text className="gray-8 text-medium text-sm">Rubix SQL</Text>
              </span>
            )}
          </div>
